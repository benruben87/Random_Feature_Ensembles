...
from common import *
from train import *
from model.cnn import ResNetConfig, EnsembleConfig
...
def _get_c(n_params):
    coeffs = np.array([2724, 377, 10 - n_params])
    c = np.max(np.roots(coeffs))
...
train_task = Vision('cifar10', batch_size=128, data_resolution=32, augment=True)
